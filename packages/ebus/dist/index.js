...
__export(index_exports, {
  ...
  ProcedureNotReadyError: () => ProcedureNotReadyError,
  err: () => err,
  initEBUS: () => initEBUS,
  ok: () => ok
});
module.exports = __toCommonJS(index_exports);
var import_erpc14 = require("@eleplug/erpc");
...
var import_erpc3 = require("@eleplug/erpc");
...
async function createPeerStack(transport, bridge, resourceManager, streamManager) {
  const t = import_erpc2.initERPC.create();
  ...
  const peerFeatures = [
    ...
    new import_erpc2.ErrorHandlingFeature(),
    new import_erpc2.PinFeature(resourceManager),
    new import_erpc2.TunnelFeature(),
    new import_erpc2.StreamFeature(streamManager),
    new import_erpc2.SerializationFeature(),
    // Protocol Handling - Client calls the other peer's forwardMessage,
    // Executor runs our own implementation.
    new import_erpc2.ProtocolHandlerFeature(),
    new import_erpc2.CallManagerFeature(),
    new import_erpc2.CallExecutorFeature(internalApiImpl),
    // Transport Adaptation
    new import_erpc2.TransportAdapterFeature(transport)
  ];
  const stack = await (0, import_erpc2.buildFeatures)(peerFeatures);
...
var BridgeManagerFeature = class {
  ...
  ebusId = (0, import_uuid.v4)();
  busEvents = new import_transport.AsyncEventEmitter();
...
var import_erpc5 = require("@eleplug/erpc");

// src/features/local/context.middleware.ts
var import_erpc4 = require("@eleplug/erpc");
...
function createContextInjectorMiddleware(validator, errorMessage) {
  return (0, import_erpc4.middleware)(async ({ meta, next, input }) => {
...
var LocalNodeManagerFeature = class {
  ...
  async registerNode(options) {
    ...
    if (options.apiFactory) {
      const t_p2p = import_erpc5.initERPC.create();
      ...
      const handlers = (0, import_erpc5.createProcedureHandlers)(api);
  ...
  async addSubscription(nodeId, topic, consumnerFactory) {
    const nodeProfile = this.localNodes.get(nodeId);
    if (!nodeProfile) throw new NodeNotFoundError(nodeId);
    const t_pubsub = import_erpc5.initERPC.create();
...
var import_erpc6 = require("@eleplug/erpc");
  ...
  createP2PClient(sourceNodeId, targetNodeId) {
    const callProcedure = (path, action, args, meta) => {
      if (action === "ask") {
        const payload = {
          type: "ask",
          callId: `${sourceNodeId}:${(0, import_uuid2.v4)()}`,
          path,
          args,
          meta
        };
    ...
    return (0, import_erpc6.buildClient)(callProcedure);
  }
...
var import_erpc7 = require("@eleplug/erpc");
  ...
  createPublisher(options) {
    return buildPublisher((topic, path, action, args, meta) => {
      const payload = action === "all" ? {
        type: "ask",
        callId: `${options.sourceNodeId}:${(0, import_uuid3.v4)()}`,
        path,
        args,
        meta
...
var import_erpc8 = require("@eleplug/erpc");
var RoutingFeature = class {
  ...
  async propagateSubscriptionChangeUpstream(updates) {
    ...
    const message = {
      kind: "sub-update",
      updates,
      correlationId: (0, import_uuid4.v4)()
    };
...
var import_erpc9 = require("@eleplug/erpc");
...
var import_erpc10 = require("@eleplug/erpc");
...
var ProtocolCoordinatorFeature = class {
  capability;
  semanticEvents = new import_transport2.AsyncEventEmitter();
  ...
  async initiateHandshake(source) {
    ...
    const handshakeMessage = {
      kind: "handshake",
      correlationId: (0, import_uuid5.v4)()
    };
...
var import_erpc11 = require("@eleplug/erpc");
...
var import_erpc13 = require("@eleplug/erpc");

// src/features/pin/pin-dispatch.handler.ts
var import_erpc12 = require("@eleplug/erpc");
var PinHandler = {
  /**
   ...
   */
  canHandle(value) {
    return typeof value === "function" && value[import_erpc12.PIN_FREE_KEY] !== void 0;
  },
  /**
...
async function createEbusInstance(parentTransport) {
  const resourceManager = new import_erpc14.ResourceManager();
  const streamManager = new import_erpc14.StreamManager();
  const features = [
  ...
  const bus = await (0, import_erpc14.buildFeatures)(features);
  return {
    ...bus.capability,
    close: bus.close
  };
