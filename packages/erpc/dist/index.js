...
__export(index_exports, {
  CallExecutorFeature: () => CallExecutorFeature,
  CallManagerFeature: () => CallManagerFeature,
  ErrorHandlingFeature: () => ErrorHandlingFeature,
  IllegalParameterError: () => IllegalParameterError,
  IllegalResultError: () => IllegalResultError,
  IllegalTypeError: () => IllegalTypeError,
  LifecycleFeature: () => LifecycleFeature,
  PIN_FREE_KEY: () => PIN_FREE_KEY,
  PIN_ID_KEY: () => PIN_ID_KEY,
  PIN_REQUEST_KEY: () => PIN_REQUEST_KEY,
  PinFeature: () => PinFeature,
  ProcedureError: () => ProcedureError,
  ProtocolHandlerFeature: () => ProtocolHandlerFeature,
  ResourceManager: () => ResourceManager,
  SerializationFeature: () => SerializationFeature,
  Serializer: () => Serializer,
  StreamFeature: () => StreamFeature,
  StreamManager: () => StreamManager,
  TransportAdapterFeature: () => TransportAdapterFeature,
  TunnelFeature: () => TunnelFeature,
  buildClient: () => buildClient,
  buildFeatures: () => buildFeatures,
  createClient: () => createClient,
  createPeer: () => createPeer,
  createPinHandler: () => createPinHandler,
  createProcedureHandlers: () => createProcedureHandlers,
  createServer: () => createServer,
  createStreamHandler: () => createStreamHandler,
  errorHandler: () => errorHandler,
  free: () => free,
  illegalTypeErrorHandler: () => illegalTypeErrorHandler,
  initERPC: () => initERPC,
...
var import_circular_buffer_js = require("circular_buffer_js");
...
var IncomingBuffer = class {
  ...
  constructor(capacity = 256) {
    this.buf = new import_circular_buffer_js.circular_buffer(capacity);
  }
  /**
...
var TunnelManager = class {
  ...
  bridgeLocalTransport(localTransport) {
    const tunnelId = (0, import_uuid2.v4)();
    ...
    if (!proxy) {
      proxy = new ProxyTransport(
        ...
        async () => {
          ...
          await hostOutgoingChannel.send({ type: "stream-tunnel", tunnelId, streamId: (0, import_uuid2.v4)(), targetEndpoint: "initiator" });
  ...
  forwardIncomingStreamFromBridge(tunnelId, localIncomingChannel) {
    const destinationProvider = (async () => {
      const hostOutgoingChannel = await this.hostOpenStream();
      await hostOutgoingChannel.send({ type: "stream-tunnel", tunnelId, streamId: (0, import_uuid2.v4)(), targetEndpoint: "receiver" });
      return hostOutgoingChannel;
    })();
...
function createStreamHandler(capability) {
  return {
    ...
    serialize(stream) {
      const handshakeId = (0, import_uuid3.v4)();
...
var ProtocolHandlerFeature = class {
  semanticEmitter = new import_transport2.AsyncEventEmitter();
  contribute() {
    return {
...
var CallManagerFeature = class {
  ...
  trackAsk(path, args, meta, kind = "erpc") {
    ...
    const callId = (0, import_uuid4.v4)();
    const { serializer, sendRawMessage } = this.capability;
    const request = {
      type: "rpc-request",
      kind,
...
var TransportAdapterFeature = class {
  transport;
  rawEmitter = new import_transport3.AsyncEventEmitter();
  controlChannel;
  closing = false;
