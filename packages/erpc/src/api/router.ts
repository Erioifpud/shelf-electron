...
import { __procedure_brand, type AskProcedure, type DynamicProcedure, type Procedure, type TellProcedure } from "./procedure";
 ...
 * @internal
 */
async function executeMiddlewareChain<TInput extends Array<unknown>, Output>(options: {
  ...
  const dispatch = async (index: number, currentOpts: { ctx: any; input: TInput; meta: any}): Promise<any> => {
    // If we've run out of middlewares, call the final procedure handler.
    if (index >= middlewares.length) {
      const finalEnv: Env<any> = { ...env, ctx: currentOpts.ctx, meta: currentOpts.meta};
      return finalHandler(finalEnv, currentOpts.input);
    }

    const middleware = middlewares[index];
    const next = (nextPartialOpts?: { ctx?: any; input?: TInput, meta?: any }) => {
...
export type ProcedureHandlers<TInput extends Array<unknown>, TOutput> = {
  ...
  handleAsk: (
    env: Env<any>, path: string, input: TInput
 ...
 */
export function isProcedure(api: Api<any, any>): api is Procedure<any, any, any> {
 ...
 */
export function createProcedureHandlers<TInput extends Array<unknown>, TOutput, TApi extends Api<TInput, TOutput>>(
  api: TApi
): ProcedureHandlers<TInput, TOutput> {
  const staticProcedureMap = new Map<string, Procedure<any, TInput, TOutput>>();
  const dynamicProcedureTrie = new Trie<DynamicProcedure<any, TInput, TOutput>>();

  /** Recursively traverses the API tree to populate the procedure maps. */
  const buildProcedureMaps = (currentApi: Api<TInput, TOutput>, prefix = ''): void => {
    if (isProcedure(currentApi)) {
      if (currentApi.type === 'dynamic') {
        dynamicProcedureTrie.insert(prefix, currentApi as DynamicProcedure<any, TInput, TOutput>);
      } else {
  ...
  const findProcedure = (path: string): { procedure: Procedure<any, any, any>; relativePath?: string[] } | undefined => {
      ...
      const result = await executeMiddlewareChain({
        ...
        finalHandler: (finalEnv, finalInput) => {
          ...
          switch (procedure.type) {
            case 'dynamic':
              return (procedure as DynamicProcedure<any, TInput, TOutput>)._handler(finalEnv, relativePath!, finalInput, type);
            case 'ask':
              return (procedure as AskProcedure<any, TInput, TOutput>)._handler(finalEnv, ...finalInput);
            case 'tell':
              return (procedure as TellProcedure<any, TInput>)._handler(finalEnv, ...finalInput);
          }
        },
      });
