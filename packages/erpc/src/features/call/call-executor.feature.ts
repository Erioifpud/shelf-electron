...
type CallExecutorRequires = ProtocolHandlerContribution &
...
> implements Feature<{}, CallExecutorRequires>
{
  private handlers: ReturnType<
    typeof createProcedureHandlers<TransferableArray, Transferable, TApi>
  >;
  ...
  constructor(api: TApi) {
    this.handlers = createProcedureHandlers<
      TransferableArray,
      Transferable,
      TApi
    >(api);
  ...
  public init(capability: CallExecutorRequires): void {
    const { semanticEmitter, serializer, sendRawMessage, isClosing } =
      capability;

    // Listen for 'ask' (request-response) calls.
    semanticEmitter.on("ask", async (message) => {
      // 1. Deserialize inputs and metadata.
      const deserializedInput = message.input.map((i) =>
        serializer.deserialize(i)
      );
      const meta = message.meta
        ? serializer.deserialize(message.meta)
        : undefined;

      // 2. Execute the procedure handler.
      const env = { ctx: undefined, meta, isClosing };
      const result = await this.handlers.handleAsk(
        env,
        message.path,
        deserializedInput
      );
      ...
      if (result) {
        ...
        const serializedOutput = serializer.serialize(
